...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toastError(error.response?.data?.detail || '注册失败')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="min-h-screen bg-surface-50 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="max-w-md w-full relative z-10">
        <div className="text-center mb-8 animate-fade-in-up">
          ...
          <p className="text-surface-500">创建你的账户</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white border border-surface-200 rounded-2xl p-8 space-y-5 shadow-lg animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
          <div>
            <label className="block text-sm font-medium text-surface-700 mb-1.5">昵称</label>
            <input
              type="text"
              value={name}
          ...
          <div>
            <label className="block text-sm font-medium text-surface-700 mb-1.5">确认密码</label>
            <input
              type="password"
